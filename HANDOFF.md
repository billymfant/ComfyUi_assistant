...
- **LOCAL PATCHES to ComfyUI-Trellis2-GGUF** (user-approved, re-apply if the pack updates; both worth PRing upstream):
  1. `nodes.py`, class `Trellis2_GGUFPostProcessAndUnWrapAndRasterizer` — author forgot `out_faces_int32 = out_faces.to(torch.int32)` when duplicating the rasterize block (single-image path has it at ~line 1218; multiview tail used it undefined at ~line 1956 → NameError). One line added before `print("Sampling attributes...")`.
  2. `trellis2_gguf/pipelines/trellis2_image_to_3d.py`, `sample_shape_slat_cascade_multiview` — LR and HR samplers never move `flow_model_lr`/`flow_model` to GPU (only offload after; single-view path does it at ~line 772). Works on fresh load, but after ANY low_vram offload cycle (e.g. a single-image run first) → "Expected all tensors to be on the same device". Added `if self.low_vram: <model>.to(self.device)` before both `.sample(...)` calls.
