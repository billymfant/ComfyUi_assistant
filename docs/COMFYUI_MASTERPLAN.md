...
- **Full Wan-Animate (face + SAM2 mask) ✅ WORKS, all on the 4080, ~64s** (`tests/test_wan_animate_full.py`). Pipeline: VHS_LoadVideo → OnnxDetectionModelLoader(**CUDAExecutionProvider**) → PoseAndFaceDetection → DrawViTPose (pose_video) + face_images; DownloadAndLoadSAM2Model(**sam2.1_hiera_base_plus**, **segmentor=video**) → Sam2Segmentation(bboxes) → GrowMaskWithBlur → BlockifyMask (character_mask) + DrawMaskOnImage (background_video); LoadImage→ImageScale (reference_image) → WanAnimateToVideo(all inputs) → CFGGuider(1)+BasicScheduler(simple,4)+lcm → SamplerCustomAdvanced → decode. Loadable `workflows/08_WAN_ANIMATE_full.json` (configured to match).
  - **Two key fixes:** (1) ONNX GPU — the embedded python had 3 conflicting onnxruntime pkgs (plain + gpu + openvino) and OpenVINO hijacked the import; cleaned to **onnxruntime-gpu only** (now 1.26.0, CUDAExecutionProvider works; cuDNN9 comes from torch/lib which ComfyUI loads). ViTPose dropped from ~94s (CPU) to part of a 64s total run. (2) SAM2 — must use a **2.1** model with **video** segmentor to accept bboxes (2.0 + single_image caused a bbox-batch IndexError).
  - Operational note: pip on this embedded python needs TEMP/TMP on the F: drive (cross-drive move error), and ComfyUI must be stopped to replace locked onnxruntime DLLs (stop via PID on :8188, relaunch `python_embeded\python.exe -s ComfyUI/main.py --windows-standalone-build --use-flash-attention`).
